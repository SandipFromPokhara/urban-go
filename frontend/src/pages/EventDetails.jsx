      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          const transformedEvent = {
            ...
            description: apiEvent.shortDescription?.en || apiEvent.description?.en || '',
            longDescription: apiEvent.description?.en || apiEvent.shortDescription?.en || 'No description available.',
            date: apiEvent.startTime,
            endDate: apiEvent.endTime,
            time: formatTimeRange(apiEvent.startTime, apiEvent.endTime),
            location: apiEvent.location?.name?.en || apiEvent.location?.city?.en || 'Helsinki',
            fullAddress: [
              apiEvent.location?.streetAddress?.en,
              apiEvent.location?.city?.en,
              apiEvent.location?.postalCode
            ].filter(Boolean).join(', '),
            image: apiEvent.images?.[0]?.url || 'https://images.unsplash.com/photo-1492684223066-81342ee5ff30?w=800',
            category: apiEvent.categories?.[0] || apiEvent.keywords?.[0]?.name?.en || '',
            tags: apiEvent.categories || [],
            organizer: apiEvent.provider?.en || apiEvent.publisher || 'Event Organizer',
            ticketPrice: apiEvent.offers?.[0]?.isFree ? 'Free Entry' : (apiEvent.offers?.[0]?.price?.en || 'Check website for pricing'),
            isFree: apiEvent.offers?.[0]?.isFree || false,
            infoUrl: apiEvent.infoUrl?.en || apiEvent.offers?.[0]?.infoUrl?.en || '',
            keywords: apiEvent.keywords?.map(kw => kw.name?.en).filter(Boolean) || []
          };
            ...
            <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-md p-6`}>
              ...
              <div className={`border-t pt-6 ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                ...
                <div 
                  ...
                  dangerouslySetInnerHTML={{ __html: event.longDescription.replace(/\n/g, '<br>') }}
                />
              </div>
